...
import { Search, Plus, Users, Briefcase, Clock, FileText, X, Check } from 'lucide-react';
...
export function HrModule({ canEdit = true }: { canEdit?: boolean }) {
  ...
  const handleAdd = (data: Partial<Employee>) => {
...
function AddEmployeeForm({ onSubmit, onCancel }: { onSubmit: (d: Partial<Employee>) => void; onCancel: () => void }) {
  const [form, setForm] = useState<Record<string, string>>({
    name: '', role: '', department: 'Sales', employment_type: 'Full-time', email: '', phone: '', start_date: '', education: '', state_of_origin: '', bank_name: '', account_number: '', emergency_contact_name: '', emergency_contact_relationship: '', emergency_contact_phone: '',
  });
